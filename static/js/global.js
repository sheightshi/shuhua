    ...
    var Calendar = function (params) {
        ...
        var defaults = {
            ...
            monthPickerTemplate:
                ...
                '<a href="#" class="link icon-only picker-calendar-prev-month"><i class="ficon ficon-leftarrow"></i></a>' +
                '<div class="current-month-value"></div>' +
                '<a href="#" class="link icon-only picker-calendar-next-month"><i class="ficon ficon-leftarrow"></i></a>' +
                '</div>',
            yearPicker: true,
            yearPickerTemplate:
                '<div class="picker-calendar-year-picker">' +
                '<a href="#" class="link icon-only picker-calendar-prev-year"><i class="ficon ficon-leftarrow"></i></a>' +
                '<span class="current-year-value"></span>' +
                '<a href="#" class="link icon-only picker-calendar-next-year"><i class="ficon ficon-leftarrow"></i></a>' +
                '</div>',
            weekHeader: true,
            // Common settings
            scrollToInput: true,
...
+ function($) {
    ...
    var Picker = function (params) {
        ...
        p.updateValue = function () {
            ...
            if (p.input && p.input.length > 0) {
                // 20180412 分类与品牌特殊处理
                if($(p.input).attr("id")=="product_type" || $(p.input).attr("id").indexOf("brand_model") >= 0){
                    if(p.value.length > 1){
                        $(p.input).val(p.value.join('>'));
                    }else{
                        $(p.input).val(p.value.join(' '));
                    }
                }else{
        ...
        p.initPickerCol = function (colElement, updateItems) {
            ...
            col.replaceValues = function (values, displayValues,code) {
                col.destroyEvents();
                col.values = values;
                col.code = code;
                ...
                /*for(var i = 0 ;i<code.length;i++){
                    col.items[i].setAttribute("code", code[i]);
                }*/
                col.calcSize();
        ...
        p.columnHTML = function (col, onlyItems) {
            ...
            if (col.divider) {
                ...
                for (var j = 0; j < col.values.length; j++) {
                	if(col.code){
                		columnItemsHTML += '<div class="picker-item" data-picker-value="' + col.values[j] + '" data-code="' + col.code[j] + '">' + (col.displayValues ? col.displayValues[j] : col.values[j]) + '</div>';
                	}else{
...
+function($){
    ...
    var PhotoBrowser = function (params) {
        ...
        var navbarTemplate = pb.params.navbarTemplate ||
                            // 不需要点击返回取消pop  直接点击标题框关闭pop
                            '<header class="bar bar-nav photo-browser-close-link ' + (pb.params.type === 'popup' ?  " close-popup" : "") + '">' +
                            '<a class="icon icon-left pull-left photo-browser-close-link' + (pb.params.type === 'popup' ?  " close-popup" : "") + '"><i class="ficon ficon-leftarrow"></i></a>' +
        ...
        var toolbarTemplate = pb.params.toolbarTemplate ||
                            '<nav class="bar bar-tab">' +
                              '<a class="tab-item photo-browser-prev" href="#">' +
                                '<i class="ficon ficon-leftarrow"></i>' +
                              '</a>' +
        ...
        // Expose
        pb.exposed = false;
        pb.toggleExposition = function () {  // 点击背景切换关闭
